 ...
 */
class AiShipFighter(sector: Sector) : AiShip(sector) {

    override val actor: Actor
        get() {
            val image = Image(MagellanGame.assets.texture("sectorview_enemy_fighter.png"))
            val imageShield = Image(MagellanGame.assets.texture("sectorview_enemy_fighter_shield.png"))
            imageShield.setColor(1f, 1f, 1f, 0f)

            val stack = Stack()
            stack.setSize(300f, 208f)
    ...
    override fun prepareRenderingOnMap() {
        ...
        spriteVessel = Sprite(MagellanGame.assets.get("enemy_fighter.png", Texture::class.java))
        spriteVessel.setSize(12f, 18f)
